package com.wzq.jz_app.widget.stickyheader;
...
public class StickyHeaderGridLayoutManager extends RecyclerView.LayoutManager implements RecyclerView.SmoothScroller.ScrollVectorProvider {
   ...
   public int getLastVisibleItemPosition() {
      return getLastVisiblePosition(StickyHeaderGridAdapter.TYPE_ITEM);
   }
   ...
   public int getLastVisibleHeaderPosition() {
      return getLastVisiblePosition(StickyHeaderGridAdapter.TYPE_HEADER);
   }
   ...
   private int getFirstVisiblePosition(int type, boolean visibleTop) {
      ...
      else if (type == StickyHeaderGridAdapter.TYPE_HEADER && mHeadersStartPosition >= getChildCount()) {
         return NO_POSITION;
      }

      int viewFrom = type == StickyHeaderGridAdapter.TYPE_ITEM ? 0 : mHeadersStartPosition;
      int viewTo = type == StickyHeaderGridAdapter.TYPE_ITEM ? mHeadersStartPosition : getChildCount();
      final int recyclerTop = getPaddingTop();
   ...
   private int getLastVisiblePosition(int type) {
      ...
      int viewFrom = type == StickyHeaderGridAdapter.TYPE_ITEM ? mHeadersStartPosition - 1 : getChildCount() - 1;
      int viewTo = type == StickyHeaderGridAdapter.TYPE_ITEM ? 0 : mHeadersStartPosition;
      final int recyclerBottom = getHeight() - getPaddingBottom();
      for (int i = viewFrom; i >= viewTo; --i) {
         final View v = getChildAt(i);
         final int top = getDecoratedTop(v);
