package com.wzq.jz_app.model.repository;
...
import com.wzq.jz_app.model.gen.BBillDao;
import com.wzq.jz_app.model.gen.BSortDao;
import com.wzq.jz_app.model.gen.DaoSession;
...
public class LocalRepository {
    ...
    public Observable<List<BBill>> getBBillByUserIdWith(String id) {
        QueryBuilder<BBill> queryBuilder = mSession.getBBillDao()
                .queryBuilder()
//                .where(BBillDao.Properties.Crdate.between(DateUtils.getMillis(date), DateUtils.getMillis(endDate)))
                .where(BBillDao.Properties.Version.ge(0))
    ...
    public List<BBill> getBBillByUserIdWithYear(String id, String year) {
        String startStr = year + "-01-00 00:00:00";
        String endStr = year + "-12-31 23:59:59";
        Date date = DateUtils.str2Date(startStr);
        Date endDate =DateUtils.str2Date(endStr);
        return mSession.getBBillDao().queryBuilder().where(BBillDao.Properties.Crdate.between(DateUtils.getMillis(date), DateUtils.getMillis(endDate)),BBillDao.Properties.Version.ge(0)).list();
    }

    public Observable<List<BBill>> getBBillByUserIdWithDay(String id, String start,String end) {
